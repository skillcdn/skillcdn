...
| `usage_clients` | The distinct clients seen at a repository on the day being counted: `client` is an HMAC of the client's address under the key of that day, which is derived from the deployment's secret and the day and stored nowhere. Rows exist until the day is folded into `usage_daily`, then they are deleted ([ADR-0010](../../docs/adr/0010-distinct-clients-by-daily-keyed-hash.md), [ADR-0027](../../docs/adr/0027-client-hashes-are-keyed-from-a-configured-secret-and-the-day.md)). | unique `(repo_id, day, client)`; `day`; `account_id` |
...
| `operator_media` | An upload for the landing showcase ([ADR-0028](../../docs/adr/0028-the-front-page-and-the-explorer-are-operator-content.md)): `bytes`, their `content_type` and `size`, keyed by the SHA-256 of the bytes (`sha`), which is the name the upload is served under and never changes. Content-addressed, so the hash is the primary key. Not tenant-scoped: it is the operator's, served to everyone. | primary key `sha` |
| `showcase_entries` | An entry of the landing showcase ([ADR-0028](../../docs/adr/0028-the-front-page-and-the-explorer-are-operator-content.md)): its `slug` (the name in the admin API and the pages), `position`, the `address` it leads to, the `width` and `height` of its media, `duration_ms` and `published` of its clip, and `texts` by language tag (the words of the card and of the example conversation, as the admin API validated them). Not tenant-scoped. | unique `slug`; `(position, slug)` |
| `showcase_media` | The upload in one `slot` of a showcase entry (`clip`, `animation`, `poster`, `reference`, `picture`, `social`). Rows go with their entry (`on delete cascade`); an upload with a row cannot be deleted (`on delete restrict`). | primary key `(entry_id, slot)`; `sha` |
| `legal_documents` | A page of the deployment's own ([ADR-0029](../../docs/adr/0029-terms-and-privacy-pages-can-be-written-into-the-deployment.md)): `kind` (`terms`, `privacy`), the `revised` date the operator states, and `texts` by language tag (a title and a Markdown body, as the admin API validated them). Not tenant-scoped. | unique `kind` |
