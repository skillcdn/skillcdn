...
| `index_entries` | One file of a snapshot: `path`, `kind` (`skill`, `manifest`, `markdown`, `json`, `other`), `size`, `blob_sha`, the owning `skill_dir`, skill or repository `name`, document `title`, `description`, `front_matter` (a skill's, with the files it `include`s and its `translations`; or a repository manifest's, with the `documents` it declares, its `language` and its `translations`), the `search` vector (null when the file is listed but not searchable), `visible` (false for a file outside the skills and the document directories: known to the index, served by nothing), `digest` and `served_size` (SHA-256 in hex and size of what the file is served as, when its bytes are stored; for a listed skill's `SKILL.md`, of the assembled document), and `listed` (true for a skill the MCP skills extension lists; the reason it is not is `unlisted` in `front_matter`). **Immutable: inserted and deleted, never updated.** | unique `(snapshot_id, path)`; `(snapshot_id, kind)`; `(snapshot_id, skill_dir)`; GIN on `search`; `account_id` |
