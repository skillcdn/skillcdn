...
**A browser.** When the deployment serves the web UI, a `GET` that accepts HTML is answered with a page: a prerendered one for the static routes, in the language the `lang` parameter names, and a shell for an address, which then asks the REST API for what it shows ([ADR-0009](adr/0009-web-ui-prerendered-per-language.md), [specs/rest.md](specs/rest.md)). Serving a page resolves and indexes nothing. The server reads the build's manifest and knows nothing else about the UI.
...
**Usage statistics** are daily counters per public repository: connections, tool calls per tool, loads per skill. Each `api` process adds up what it sees and writes the totals every few seconds with an additive upsert, so replicas need no coordination and a request never waits for a write. No client address, user agent or other identifier is stored, and repositories that are not public are not counted. They feed rankings; they are not the `UsageSink` port, which carries metering events for whoever operates a deployment.
