...
- [x] `web`: landing page and explorer, in English and Korean, prerendered per language with metadata, structured data, social-preview images and `llms.txt` ([ADR-0009](adr/0009-web-ui-prerendered-per-language.md)). Design decisions made with the maintainers: a restrained developer-tool look, one blue accent, dark only, a placeholder symbol until there is a logo
...
- [x] Search-console verification and analytics tags from configuration (`GOOGLE_SITE_VERIFICATION`, `GOOGLE_ANALYTICS_ID`), with the content security policy widened for exactly them
