...
- [x] Landing copy that promises rather than explains, in both languages; structured data for the site, the clip and the questions, and an `llms.txt` that says what the site is in one sentence; files served with byte ranges, which video on phones needs
