...
| [0010](0010-distinct-clients-by-daily-keyed-hash.md) | Distinct clients are counted by a keyed hash of the address under a key that dies with the day | Accepted; points 2 and 3, the key made and stored in the database, are superseded by 0027 |
...
| [0027](0027-client-hashes-are-keyed-from-a-configured-secret-and-the-day.md) | Client hashes are keyed from a configured secret and the day, so the database alone unmasks nothing | Accepted |
| [0028](0028-the-front-page-and-the-explorer-are-operator-content.md) | The front page shows a showcase the operator writes through the admin API, with uploaded media stored by hash; the explorer keeps its own featured list; the reference repository is the default of each | Accepted |
| [0029](0029-terms-and-privacy-pages-can-be-written-into-the-deployment.md) | The terms of service and the privacy policy can be written into the deployment through the admin API and are served at `/terms` and `/privacy`, before any configured URL | Accepted |
| [0030](0030-pictures-in-repository-markdown-are-shown.md) | Pictures in repository Markdown are shown, from their https URL or from the git host's raw copy, lazily and without a referrer | Accepted |
