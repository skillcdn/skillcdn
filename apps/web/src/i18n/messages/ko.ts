...
export const ko: Messages = {
  ...
  mount: {
    ...
    license: "라이선스",
    verified: "인증됨",
    verifiedHint: "저장소 소유자 또는 이 사이트 운영자가 보증한 저장소입니다.",
    unverified: "미확인",
    unverifiedHint:
    ...
    browse: {
      ...
      holds: (skills: number, documents: number) => {
        const parts = [
          ...(skills === 0 ? [] : [`스킬 ${skills}개`]),
          ...(documents === 0 ? [] : [`문서 ${documents}개`]),
        ];
        return parts.length === 0 ? "아직 스킬이 없어요" : parts.join(" · ");
      },
      nested: (count: number) => `안에 스킬 ${count}개`,
      describedOnly: "설명만 제공",
      describedOnlyHint:
        "라이선스가 SkillCDN에 허용하는 것은 이 스킬이 있다고 알리는 것까지이며, 내용을 전달하는 것은 아닙니다.",
    },
    search: {
      label: "이 폴더와 하위 폴더 검색",
      placeholder: "이 폴더에서 검색…",
      hint: "스킬과 문서의 원문에서 찾습니다. 화면용 번역은 검색되지 않아요.",
      submit: "검색",
      clear: "지우기",
  ...
  skill: {
    ...
    licenseFile: "GitHub에서 라이선스 파일 읽기",
    describedOnly:
