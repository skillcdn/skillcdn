...
5. The MCP handler builds a server for this one request, bound to the mount ([ADR-0006](../../docs/adr/0006-mcp-sdk-v2-per-request-servers.md)). `browse`, `search` and `get_skill` wait for the index within a budget; `read_file` returns indexing until publication policy is known. Optional README introductions guide discovery, while complete skill and file text is paged to fit the MCP response budget ([tools](../../docs/specs/tools.md)).
