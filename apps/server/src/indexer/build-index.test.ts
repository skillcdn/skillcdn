...
describe("repository publication and link indexing", () => {
  ...
  it("follows shared, recursive and cyclic links without adding catalog/search documents", async () => {
    ...
    expect(result.byPath.get("README.md")?.visible).toBe(false);
  ...
  it("keeps hidden ancestor boundaries and valid skills under a broken manifest", async () => {
    const result = await index({
    ...
    expect(result.byPath.get(".agents/skills/read/SKILL.md")?.kind).toBe("skill");
    expect(result.byPath.get(".agents/skills/read/references/local.md")?.visible).toBe(true);
